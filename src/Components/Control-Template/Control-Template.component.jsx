...
const ControlTemplate = ({ submitSpell, spell_input, hints, headerText, level, handleChange }) => { //they are the only unique things to each Control //level comes from reducer now
        ...
        <section>
            ...
            <div className = "control_question">
                ...
                <div className = "control_form">
                    ...
                    <p>display: flex;</p>
                    <section>
                        <textarea className = "control_input" type="input" value={spell_input} //to clear the textarea with redux
                                  onChange={handleChange} autoFocus={true} required={true}/>
                        <p>&#125;</p>
                        <div className = "control_submit">
                            <button className="record_btn" onClick={()=>submitSpell(solutionNum)}>Next</button>
                        </div>
                    </section>
                </div>
...
const mapDispatchToProps = dispatch => ({
    ...
    submitSpell: (solutionNum) => dispatch(OnSpellSubmit(solutionNum)) //this is not a real action// we're using it to leverage thunk//every time we use an action which is not an obj form, redux thunk middleware intervene
});
